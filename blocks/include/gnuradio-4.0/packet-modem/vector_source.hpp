...
public:
    ...
    VectorSource(const std::vector<T>& data, bool repeat = false)
        : d_vector(data), d_repeat(repeat)
    {
        if (d_vector.empty()) {
    ...
    gr::work::Status processBulk(gr::PublishableSpan auto& outSpan)
    {
        ...
        d_position = static_cast<size_t>(outSpan.end() - output);
        std::ranges::copy(d_vector | std::views::take(d_position), output);
        output += static_cast<ssize_t>(d_position);
